...
public:
  TrackOnlineDnnSelector(const edm::ParameterSet&);
  ~TrackOnlineDnnSelector() override = default;
...
void TrackOnlineDnnSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (beamSpotIn.isValid()) {
    ...
    edm::LogWarning("TrackOnlineDnnSelector") << " Invalid handle for beam spot features in tracks input collection";
  ...
  for (int64_t tkIndex = 0; tkIndex < nTracks; ++tkIndex) {
    ...
    for (auto it = track.recHitsBegin(); it != track.recHitsEnd(); ++it) {
      ...
      if (hitIndex >= maxRecHits_) {
        edm::LogWarning("TrackOnlineDnnSelector") << " Track " << tkIndex << " has more (" << track.recHitsSize()
                                                  << ") than " << maxRecHits_ << " recHits, skipping the rest.";
        break;
      }
...
void TrackOnlineDnnSelector::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::FileInPath>(
          "onnxModelPath",
          edm::FileInPath("RecoTracker/FinalTrackSelectors/data/TrackOnlineDnnSelector/track_classifier.onnx"))
  ...
  desc.add<edm::InputTag>("tracksSrc", edm::InputTag("hltPhase2PixelTracks"));
